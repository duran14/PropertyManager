...
Last updated: 2026-07-11 23:05, America/Vancouver.
...
- Branch: `main`
- Latest confirmed feature commit before this handoff document: pending current commit with document intake, Obsidian export, real GLM adapter, lead assignment workflow, and stable webhook targets.

Important recent commits:

- pending: document intake, Obsidian export, real GLM adapter, lead assignment workflow, and stable webhook targets
...
- Documents are stored in the `knowledge_documents` table and exposed by `GET/POST /documents`.
...
The chatbot prompt also includes tenant onboarding policies/instructions and the latest uploaded knowledge documents. If `ZAI_API_KEY` is configured, the factory uses `GlmRealAdapter`; otherwise it keeps the existing mock adapter.
...
17. Generate the Obsidian export and confirm company, property, and document Markdown previews are shown.
18. Open a Lead Detail page, assign a staff owner, save workflow, and confirm the values persist.
19. Confirm `/webhook-config` returns stable webhook URLs based on `API_URL`.
...
2. Deepen property intake/onboarding.
   - Add durable object storage for document binaries; current upload is a DB-backed prototype.
