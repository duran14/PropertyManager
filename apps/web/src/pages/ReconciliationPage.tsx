...
const KIND_LABELS: Record<string, string> = {
  missing_in_qbo: 'Missing in QuickBooks',
  missing_in_buildium: 'Missing in Buildium',
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Daily Buildium, bank, and QuickBooks matching. Discrepancies are unresolved gaps.
          </p>
        </div>
        <button
      ...
      {runMutation.isSuccess && (
        <div className="mb-4 rounded-md bg-blue-50 border border-blue-200 px-3 py-2 text-sm text-blue-700">
          Reconciliation complete: {runMutation.data.reconciledCount} matched,{' '}
          {runMutation.data.discrepancyCount} discrepancies. Balance is{' '}
          {runMutation.data.balanced ? 'balanced' : 'out of balance'}.
        </div>
      )}

      <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-200 bg-slate-50">
          <h2 className="font-medium">Open discrepancies ({discrepancies.length})</h2>
        </div>
        <table className="w-full text-sm">
          <thead className="text-slate-600 text-xs uppercase">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-medium">Batch date</th>
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {!isLoading && discrepancies.length === 0 && (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-400">No discrepancies. Everything is balanced.</td></tr>
            )}
            {discrepancies.map((d) => (
